...
func (f *Format) printSlice(val reflect.Value) {
	...
	fieldIsStringerOrError := make([]bool, numFields)
	for i := 0; i < numFields; i++ {
		t := elemType.Field(i).Type
		if t.Implements(stringerType) || t.Implements(errorType) {
			fieldIsStringerOrError[i] = true
		}
	}
	...
	for i := 0; i < val.Len(); i++ {
		...
		for j := 0; j < numFields; j++ {
			...
			f.writeValue(w, item.Field(j), fieldIsStringerOrError[j])
		}
		_, _ = w.Write(newlineBytes)
	}
	...
	for i := 0; i < val.NumField(); i++ {
		...
		if fieldType == jsonRawMessageType {
			...
			for j := 0; j < fieldValue.NumField(); j++ {
				...
				f.writeValue(w, nestedFieldValue, nestedFieldValue.Type().Implements(stringerType) || nestedFieldValue.Type().Implements(errorType))
				_, _ = fmt.Fprintln(w)
			}
		} else {
			_, _ = fmt.Fprintf(w, "%s\t", fieldName)
			f.writeValue(w, fieldValue, fieldValue.Type().Implements(stringerType) || fieldValue.Type().Implements(errorType))
			_, _ = fmt.Fprintln(w)
		}
	}
