...
func (f *Format) writeValue(w io.Writer, v reflect.Value, isStringerOrError bool) {
	// If the value implements fmt.Stringer or error, let fmt handle it to preserve custom formatting.
	// We accept the pre-calculated boolean to avoid expensive reflection checks in loops.
...
func (f *Format) printTabularData(tabularData TabularData) {
	...
	for i, field := range tabularData.Fields {
		if sf, ok := elemType.FieldByName(field.Field); ok {
			...
			// Pre-calculate interface implementation checks
		...
		for j := range tabularData.Fields {
			...
			if fieldVal.IsValid() {
				if isJsonRawMessage[j] {
					...
					// Optimize: use Bytes() instead of Interface() to avoid allocation
					// logic kept as is but potentially optimizable.
					// For this task, we focus on writeValue signature change integration.
					...
					f.writeValue(w, fieldVal, isStringerOrError[j])
				}
			}
		}
		_, _ = w.Write(newlineBytes)
...
func (f *Format) printSlice(val reflect.Value) {
	...
	// Optimization: Pre-calculate Stringer/Error implementation status to avoid repeated reflection checks
	isStringerOrError := make([]bool, numFields)
	for i := 0; i < numFields; i++ {
		field := elemType.Field(i)
		isStringerOrError[i] = field.Type.Implements(stringerType) || field.Type.Implements(errorType)
	}
	...
	for i := 0; i < val.Len(); i++ {
		...
		for j := 0; j < numFields; j++ {
			...
			f.writeValue(w, item.Field(j), isStringerOrError[j])
		}
		_, _ = w.Write(newlineBytes)
	}
	...
	for i := 0; i < val.NumField(); i++ {
		...
		if fieldType == jsonRawMessageType {
			...
			for j := 0; j < fieldValue.NumField(); j++ {
				...
				isStrOrErr := nestedTyp.Field(j).Type.Implements(stringerType) || nestedTyp.Field(j).Type.Implements(errorType)
				_, _ = fmt.Fprintf(w, "\t%s\t", nestedFieldName)
				f.writeValue(w, nestedFieldValue, isStrOrErr)
				_, _ = fmt.Fprintln(w)
			}
		} else {
