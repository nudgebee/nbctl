...
// isStringerOrError should be pre-calculated by the caller to avoid repeated reflection lookups in loops.
	...
	for i := 0; i < val.NumField(); i++ {
		...
		if fieldType == jsonRawMessageType {
			...
			for j := 0; j < fieldValue.NumField(); j++ {
				...
				nestedIsStringerOrError := nestedField.Type.Implements(stringerType) || nestedField.Type.Implements(errorType)
				f.writeValue(w, nestedFieldValue, nestedIsStringerOrError)
				_, _ = fmt.Fprintln(w)
			}
		} else {
