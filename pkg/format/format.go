...
func (f *Format) writeValue(w io.Writer, v reflect.Value, isStringerOrError bool) {
	...
	// We use the pre-calculated boolean to avoid expensive reflection checks in the loop.
	...
	for i := 0; i < val.NumField(); i++ {
		...
		if fieldType == jsonRawMessageType {
			...
			for j := 0; j < fieldValue.NumField(); j++ {
				...
				nestedIsStringerOrError := nestedFieldType.Implements(stringerType) || nestedFieldType.Implements(errorType)
