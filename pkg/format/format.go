	...
	for i := 0; i < val.NumField(); i++ {
		...
		if fieldType == jsonRawMessageType {
			...
			for j := 0; j < fieldValue.NumField(); j++ {
				...
				isNestedStringerOrError := nestedFieldType.Implements(stringerType) || nestedFieldType.Implements(errorType)
				f.writeValue(w, nestedFieldValue, isNestedStringerOrError)
