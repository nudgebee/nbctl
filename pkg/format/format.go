...
func (f *Format) printSlice(val reflect.Value) {
	...
	for i := 0; i < numFields; i++ {
		...
		if field.Type.Implements(stringerType) || field.Type.Implements(errorType) {
			isStringerOrError[i] = true
		}
	}
	_, _ = w.Write(newlineBytes)
	...
	for i := 0; i < val.NumField(); i++ {
		...
		if fieldType == jsonRawMessageType {
			...
			for j := 0; j < fieldValue.NumField(); j++ {
				...
				f.writeValue(w, nestedFieldValue, nestedTyp.Field(j).Type.Implements(stringerType) || nestedTyp.Field(j).Type.Implements(errorType))
				_, _ = fmt.Fprintln(w)
			}
		} else {
