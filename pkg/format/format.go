...
// isStringerOrError should be pre-calculated by the caller to avoid repeated reflection checks.
func (f *Format) writeValue(w io.Writer, v reflect.Value, isStringerOrError bool) {
	...
	// We use the pre-calculated flag to skip the expensive Implements check.
...
func (f *Format) printTabularData(tabularData TabularData) {
	...
	for i, field := range tabularData.Fields {
		if sf, ok := elemType.FieldByName(field.Field); ok {
			...
			if sf.Type.Implements(stringerType) || sf.Type.Implements(errorType) {
				isStringerOrError[i] = true
			}
		}
	}
...
func (f *Format) printSlice(val reflect.Value) {
	...
	for i := 0; i < numFields; i++ {
		...
		if ft.Implements(stringerType) || ft.Implements(errorType) {
			isStringerOrError[i] = true
		}
	}
	...
	for i := 0; i < val.NumField(); i++ {
		...
		if fieldType == jsonRawMessageType {
			...
			for j := 0; j < fieldValue.NumField(); j++ {
				...
				isNestedStringer := nestedTyp.Field(j).Type.Implements(stringerType) || nestedTyp.Field(j).Type.Implements(errorType)
				f.writeValue(w, nestedFieldValue, isNestedStringer)
				_, _ = fmt.Fprintln(w)
			}
		} else {
			_, _ = fmt.Fprintf(w, "%s\t", fieldName)
			isStringerOrError := fieldType.Implements(stringerType) || fieldType.Implements(errorType)
			f.writeValue(w, fieldValue, isStringerOrError)
			_, _ = fmt.Fprintln(w)
		}
	}
